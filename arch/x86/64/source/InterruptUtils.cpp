...
extern "C" void syscallHandler()
{
  ...
  if(shouldCancel) {
    auto thread = static_cast<UserThread*>(currentThread);
    debug(SYSCALL, "T[%ld] CANCELLATION OF THREAD happening in syscallHandler()\n", thread->getTID());
...
extern "C" void errorHandler(size_t num, size_t eip, size_t cs, size_t spurious)
{
  ...
  if (userspace)
  {
    assert(currentThread->t_loader_ && "User Threads need to have a Loader");
    ...
    deb = currentThread->t_loader_->getDebugInfos();
