...
int backtrace_user(pointer *call_stack, int size, Thread *thread, bool /*use_stored_registers*/)
{
  ...
  StackFrame *CurrentFrameI = (StackFrame*)thread->t_loader_->arch_memory_.checkAddressValid((pointer)rbp);
      ...
      ADDRESS_BETWEEN(CurrentFrameI->return_address, StartAddress, EndAddress))
  {
    ...
    CurrentFrameI = (StackFrame*)thread->t_loader_->arch_memory_.checkAddressValid((pointer)CurrentFrameI->previous_frame);
  }
